...
if (function_exists('getProdutosPaginados')) {
  $dados = getProdutosPaginados('brincos', $pagina, $itensPorPagina);
  $produtos = $dados['produtos'];
  $totalPaginas = $dados['total_paginas'];
} else {
  // Fallback caso a função nova não exista
  $produtos = getProdutosPorCategoria('brincos', 100);
  $totalPaginas = 1;
}
?>
<!DOCTYPE html>
  ...
  <section class="colecao-section" style="padding: 40px 20px;">
    ...
    <div class="colecao-container">
      <?php if (!empty($produtos)): ?>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <p style="text-align:center; width:100%;">Nenhum produto disponível no momento.</p>
      <?php endif; ?>
    </div>
